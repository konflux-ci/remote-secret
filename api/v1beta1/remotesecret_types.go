...
type TargetStatus struct {
	...
	// DeployedSecret contains the status information about the linked secret deployed in the target
	// +optional
	DeployedSecret *DeployedSecretStatus `json:"deployedSecret,omitempty"`
}

type DeployedSecretStatus struct {
	Labels      map[string]string `json:"labels,omitempty"`
	Annotations map[string]string `json:"annotations,omitempty"`
...
func (ts TargetStatus) ToTargetKey() TargetKey {
	...
	if ts.DeployedSecret != nil {
		secretName = ts.DeployedSecret.Name
	}
	secretGenerateName = ""

	// keep the backwards compatibility when upgrading from an RS with status generated by the older version of the controller.
	if secretName == "" {
		secretName = ts.SecretName
	}

	if secretName == "" && ts.ExpectedSecret != nil {
