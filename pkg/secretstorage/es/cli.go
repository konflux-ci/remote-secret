...
func NewESSecretStorage(ctx context.Context, providerConfJSON string) (secretstorage.SecretStorage, error) {
	providerConf := &es.SecretStoreProvider{}
	err := json.Unmarshal([]byte(providerConfJSON), providerConf)
	if err != nil {
		return nil, fmt.Errorf("failed deserialize provider config: %w", err)
	}
